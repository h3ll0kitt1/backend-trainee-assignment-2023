...
* Для логгирования использовался пакет go.uber.org/zap; 
...
**Параметры:**
* `user_list` - список идентификаторов пользователей, для которых необходимо выгрузить историю
* `days` - период в днях за который надо выгрузить историю 
